...
- Known pre-existing quirk (not fixed here): `POST /api/clients` uses `insert ... returning`, and the `clients` SELECT policy (`can_access_client`) does not yet see a just-created row for a non-owner/admin therapist (their `client_access` row is written afterward), so `RETURNING` raises an RLS error for them. Owners/admins are unaffected. Fixing it means letting the creator/primary therapist view their own client.
