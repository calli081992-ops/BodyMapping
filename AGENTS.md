...
### Database / migrations
- Migrations live in `supabase/migrations/` (`001`–`006`) and target Supabase Postgres. They assume the Supabase-managed baseline exists (`auth` schema, `auth.uid()`, and the `anon`/`authenticated`/`service_role` roles).
